...
from transformers import  AutoModel, TimesformerModel
...
class Model3D(nn.Module):
    ...
    def __init__(self, pretrained_model_name: str = "facebook/timesformer-base-finetuned-k600", 
        ...
        # Define unified classification head structure (two-layer linear + ReLU + Dropout)
    ...
    def _init_classifier_weights(self):
        """Initialize weights of the classification head, following best practices"""
        for module in self.classifier:
            if isinstance(module, nn.Linear):
                # Initialize weights of linear layers with normal distribution, biases initialized to 0
    ...
    def forward(
        ...
        """
        Forward pass function, returns only logits
        Args:
            bus_images/cdfi_images/clinical_features/original_features: Reserved parameters (compatible with original inputs)
            ceus_features: CEUS video features, different model input formats:
                - Timesformer: (B, 16, C, H, W) or (B, C, H, W) (single frame)
                - Swin3D/R2+1D: (B, 16, C, H, W)
        Returns:
            logits: Tensor of shape [batch_size, num_classes]
        """
        # 1. Handle different model input formats and feature extraction
        if self.model_name == "timesformer":
            ...
            # Extract [CLS] token feature (B, hidden_size)
...
class DINOv3Classifier(nn.Module):
    def __init__(self, pretrained_model_name, num_classes=2, feature_dim=768,
        ...
        self.tokshift = TokShiftFusion(dim=feature_dim, shift_ratio=0.2, agg="mlp")
        self.PTA = PTAFusion(agg='mlp')
        self.TCStyleContext = TCStyleContextFusion(dropout=0.5)
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for batch in progress_bar:
        ...
        predictions = (outputs > 0.5).float()  # Threshold 0.5 for binary classification
...
# Add a new test function to record detailed information for each patient
def evaluate_with_patient_info(model, dataloader, criterion, device, save_dir, mode='test'):
    """
    Unified evaluation function for training set/validation set/test set/sub-validation set/external test set
    ...
    """
    # Set description and file name based on mode
    mode_descriptions = {
    ...
    with torch.no_grad():
        ...
        for batch in progress_bar:
            ...
            # Record whether prediction is correct
...
# Retain compatibility, define alias
def test_with_patient_info(model, dataloader, criterion, device, save_dir):
    """Test model and record detailed information for each patient (compatibility interface)"""
...
def train_with_patient_info(model, dataloader, criterion, device, save_dir):
    """Training set prediction and record detailed information for each patient (compatibility interface)"""
...
def validate_with_patient_info(model, dataloader, criterion, device, save_dir):
    """Validation set prediction and record detailed information for each patient (compatibility interface)"""
...
def train_with_hyperparams(params, device, train_loader, sub_val_loader, test_loader, external_test_loader, save_dir, args):
    """Train model with specific hyperparameters, including training set, sub-validation set, test set, and external test set"""
    ## ------- Model ------- ##
    if args.modality == 'CEUS_3D_R3D':
        ...
        model = Model3D(pretrained_model_name="./ckpt/pretrained_weights/models--facebook--timesformer-base-finetuned-k600/snapshots/15a98085a5002abc11024d10e5823bb934c24be6",
    ...
    # Set initial value of best validation metric based on save_sign
    if args.save_sign == "AUC":
        best_val_metric = 0.0  # Best AUC is larger better
        metric_name = "AUC"
        is_better = lambda current, best: current > best + 1e-6  # AUC is larger better
        patience = 5  # Early stopping patience
        patience_counter = 0  # Early stopping counter
    elif args.save_sign == "Loss":
        best_val_metric = float('inf')  # Best Loss is smaller better
        metric_name = "Loss"
        is_better = lambda current, best: current < best - 1e-6  # Loss is smaller better
        patience = 5  # Early stopping patience
        patience_counter = 0  # Early stopping counter
    elif args.save_sign == "Loss_AUC":
    ...
    print(f"\n{'='*60}")
    print(f"Starting training, hyperparameters: {params}")
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Learning Rate: Backbone={current_lrs[0]:.2e}, Classifier={current_lrs[1]:.2e}, Other={current_lrs[1]:.2e}")
        print(f"Train     - Loss: {train_loss:.4f}, AUC: {train_auc:.4f}, ACC: {train_acc:.4f}, "
              f"Sensitivity: {train_tpr:.4f}, Specificity: {train_tnr:.4f}, Precision: {train_precision:.4f}, F1: {train_f1:.4f}")
        print(f"Sub_Val   - Loss: {sub_val_loss:.4f}, AUC: {sub_val_auc:.4f}, ACC: {sub_val_acc:.4f}, "
              f"Sensitivity: {sub_val_tpr:.4f}, Specificity: {sub_val_tnr:.4f}, Precision: {sub_val_precision:.4f}, F1: {sub_val_f1:.4f}")
        print(f"Test      - Loss: {test_loss:.4f}, AUC: {test_auc:.4f}, ACC: {test_acc:.4f}, "
              f"Sensitivity: {test_tpr:.4f}, Specificity: {test_tnr:.4f}, Precision: {test_precision:.4f}, F1: {test_f1:.4f}")
        print(f"External  - Loss: {external_test_loss:.4f}, AUC: {external_test_auc:.4f}, ACC: {external_test_acc:.4f}, "
              f"Sensitivity: {external_test_tpr:.4f}, Specificity: {external_test_tnr:.4f}, Precision: {external_test_precision:.4f}, F1: {external_test_f1:.4f}")
        ...
        # Early stopping logic (based on sub-validation set metric)
        if args.save_sign == "AUC":
            ...
            # 1. Early stopping judgment (based on Loss)
    ...
    # Test set evaluation (on best sub-validation set model)
    ...
    # External test set evaluation (on best sub-validation set model)
    ...
    with open(os.path.join(save_dir, 'final_results.txt'), 'w') as f:
        ...
        f.write(f"Sensitivity: {train_tpr_final:.4f}\n")
        f.write(f"Specificity: {train_tnr_final:.4f}\n")
        ...
        f.write(f"Sensitivity: {sub_val_tpr_final:.4f}\n")
        f.write(f"Specificity: {sub_val_tnr_final:.4f}\n")
        ...
        f.write(f"Sensitivity: {test_tpr_final:.4f}\n")
        f.write(f"Specificity: {test_tnr_final:.4f}\n")
        ...
        f.write(f"Sensitivity: {external_test_tpr_final:.4f}\n")
        f.write(f"Specificity: {external_test_tnr_final:.4f}\n")
...
def build_argparser():
    ...
    parser.add_argument(
        ...
        choices=["Avg", "Max", "Attention", 
                 "HTCM", "Tokshift", "PTA",
                 "TCStyleContext"],
        help="Fusion method for logging/saving"
    )
    parser.add_argument(
        "--fold_num",
        type=int,
    ...
    # Add multi-GPU support parameter
    parser.add_argument(
        "--multi_gpu",
        action="store_true",
...
def main():
    ...
    if args.modality == "CEUS_2D_dinov3_HTCM_v5_random":
        image_dirs['ceus'] = "./datasets/CEUS_select_frames/1_Crop_same_shape_avi_random_16frames"
    elif args.modality == "CEUS_2D_dinov3_HTCM_v5_uniform":
        image_dirs['ceus'] = "./datasets/CEUS_select_frames/1_Crop_same_shape_avi_uniform_16frames"

    print(f"Using CEUS dir : \n{image_dirs['ceus']}")
    clinical_data_path = "./datasets/Clin/MVI-HCC-samples.xlsx"

    # Create datasets (Note: do not fix batch_size here)
    # Use MultiModelDataset, supports four datasets:
    ...
    # Sub-validation set (split from training set)f
    ...
    for idx, params_tuple in enumerate(all_param_combinations):
        ...
        # Adjust batch size based on whether multi-GPU is used
        if args.multi_gpu:
        ...
        print(f"\n{'='*80}")
        print(f"Parameter combination {idx+1}/{len(all_param_combinations)}")
        ...
        print(f"Best sub-validation set AUC so far: {best_overall_auc:.4f} (Experiment directory: {best_exp_dir})")
    ...
    print(f"  Test set Sensitivity: {best_results['test_tpr']:.4f}")
    print(f"  Test set Specificity: {best_results['test_tnr']:.4f}")
    ...
    print(f"  External test set Sensitivity: {best_results['external_test_tpr']:.4f}")
    print(f"  External test set Specificity: {best_results['external_test_tnr']:.4f}")
