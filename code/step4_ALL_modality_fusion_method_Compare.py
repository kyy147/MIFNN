'''
Three imaging modality fusion experiments
Dataset split: Five-fold cross-validation for training set, external test
...
class DINOv3Classifier(nn.Module):
    def __init__(self, pretrained_model_name, num_classes=1, feature_dim=768,
        ...
        # Classifier
        classifier_input_dim = feature_dim * 3 if fusion_way in ['concate', 'bottle_neck'] else feature_dim
        if fusion_way == 'transformer_ablation':
            classifier_input_dim = feature_dim * 2
        elif fusion_way == 'transformer':
            classifier_input_dim = feature_dim * 2
        ...
        # learnable modality embedding (BUS, CDFI, CEUS)
        ...
        ## ------ bottle_neck ------ 
        self.bottle_neck = VectorAttentionBottleneck(d_model=768, num_heads=4, bottleneck_size=768)
        ## ------ bilinear_proj ------ 
        if self.fusion_way == 'bilinear':
            hidden_dim = 128
            self.bus_proj = nn.Linear(768, hidden_dim)
            self.cdfi_proj = nn.Linear(768, hidden_dim)
            self.ceus_proj = nn.Linear(768, hidden_dim)
            self.bilinear_proj1 = nn.Bilinear(hidden_dim, hidden_dim, hidden_dim)
            self.bilinear_proj2 = nn.Bilinear(hidden_dim, hidden_dim, hidden_dim)
            self.relu = nn.ReLU()
            self.dropout = nn.Dropout(0.2)
            self.fusion_out = nn.Linear(hidden_dim, 768)
    ...
    def _setup_clinical_backbone(self, pretrained_clinical_path):
        """
        Load pretrained weights to clinical_backbone, then freeze parameters
        Using AutoModel as feature extractor (without classification head)
        ...
        if pretrained_clinical_path is not None:
            ...
            for k, v in pretrained_dict.items():
                ...
                if k.startswith('clinical_backbone.'):
                    ...
                    # Try direct usage (assuming pretrained weights are already bert weights without prefix)
                    target_key = k
                ...
                if target_key is not None and target_key in clinical_state_dict:
                    ...
                    if target_key is not None:
                        skipped_keys.append(f"{k} -> {target_key} (not exist)")
            ...
            if skipped_keys:
                print(f"Skipped Clinical parameters ({len(skipped_keys)} items):")
                for key in skipped_keys[:5]:
                    print(f"  - {key}")
                if len(skipped_keys) > 5:
                    print(f"  ... {len(skipped_keys) - 5} more parameters skipped")
        else:
            print("No pretrained_clinical_path provided, clinical_backbone uses pretrained weights without fine-tuned weights")

        # Freeze all clinical_backbone parameters
        for param in self.clinical_backbone.parameters():
            param.requires_grad = False
        print("clinical_backbone parameters frozen, used only as feature extractor")
    ...
    def _setup_ceus_backbone(self, pretrained_ceus_path):
        """
        Load pretrained weights to backbone and blood_encoder (HTCM)
        ...
        if skipped_keys:
            print(f"Skipped CEUS parameters ({len(skipped_keys)} items):")
            for key in skipped_keys[:5]:
                print(f"  - {key}")
            if len(skipped_keys) > 5:
                print(f"  ... {len(skipped_keys) - 5} more parameters skipped")
    ...
    def info_nce_loss(self, dynamic_feat, static_feat, temperature=0.07):
        ...
        # Ensure contrastive_loss is scalar
    ...
    def forward(self, bus_image, cdfi_image, input_ids, attention_mask, x):
        ...
        if self.fusion_way == 'transformer_ablation':
            dynamic_feat = sequence_features.mean(dim=1)
        else:
            dynamic_feat = self.blood_encoder(sequence_features)    

        ## ------ Clinical BERT ------ ##
        # Use clinical_backbone (Bio_ClinicalBERT) to process input_ids and attention_mask
        ...
        ## ------ Modality Fusion ------ ##
        # ---- addition
        if self.fusion_way == 'addition':
            fused_repr = bus_features + cdfi_features + dynamic_feat
        # ---- avg
        elif self.fusion_way == 'avg':
            fused_repr = (bus_features + cdfi_features + dynamic_feat) / 3.0
        # ---- concate
        elif self.fusion_way == 'concate':
            fused_repr = torch.cat([bus_features, cdfi_features, dynamic_feat], dim=1)
        # ---- bottle neck
        elif self.fusion_way == 'bottle_neck':
            fused_repr = self.bottle_neck(bus_features, cdfi_features, dynamic_feat)
        elif self.fusion_way == 'transformer':
            B = bus_features.shape[0]
            cls_tokens = self.cls_token.expand(B, -1, -1)                  # [B, 1, 768]

            # self.modality_embeddings: [3, 1, 768] -> expand to [B, 3, 768]
            mod_embeds = self.modality_embeddings.expand(-1, B, -1).transpose(0, 1)  # [B, 3, 768]
            ...
            # todo: for all fusion experiments
            fused_repr = torch.cat([fused_repr, clinical_features], dim=-1)
        elif self.fusion_way == 'bilinear':
            bus_p = self.bus_proj(bus_features)
            cdfi_p = self.cdfi_proj(cdfi_features)
            ceus_p = self.ceus_proj(dynamic_feat)

            bc_fused = self.dropout(self.relu(self.bilinear_proj1(bus_p, cdfi_p)))
            fused_repr = self.dropout(self.relu(self.bilinear_proj2(bc_fused, ceus_p)))
            fused_repr = self.fusion_out(fused_repr)
        elif self.fusion_way == 'transformer_ablation':
            B = bus_features.shape[0]
            cls_tokens = self.cls_token.expand(B, -1, -1)                  # [B, 1, 768]
...
def train_with_patient_info(model, dataloader, criterion, device, save_dir):
    """Training set prediction and record detailed patient information (compatibility interface)"""
...
def train_with_hyperparams(params, device, train_loader, sub_val_loader, test_loader, external_test_loader, save_dir, args):
    ...
    if args.save_sign == "AUC":
        ...
        metric_name = "AUC"
        is_better = lambda current, best: current > best + 1e-6  # AUC is higher better
        patience = 5  # Early stopping patience
        patience_counter = 0  # Early stopping counter
    elif args.save_sign == "Loss":
        best_val_metric = float('inf')  # Best Loss is lower better
        metric_name = "Loss"
        is_better = lambda current, best: current < best - 1e-6  # Loss is lower better
        patience = 5  # Early stopping patience
        patience_counter = 0  # Early stopping counter
...
def build_argparser():
    ...
    parser.add_argument(
        ...
        choices=["Avg", "Max", "Attention", 
                 "HTCM", "Tokshift", "PTA", "TCStyleContext",
                 "addition", "avg", "concate", "bottle_neck", "transformer", "bilinear",
...
def main():
    ...
    hyperparam_space = {
        # Different modules use different learning rates
        'lr_backbone': [1e-6, 2e-6],
        'lr_static_backbone': [1e-6],
        'lr_transformer': [1e-4],
        'lr_classifier': [1e-4, 1e-5],
        'lr_other': [1e-5],
        ...
        'contrastive_weight': [0.1],  # Contrastive loss weight, controls contrastive loss magnitude
    }
