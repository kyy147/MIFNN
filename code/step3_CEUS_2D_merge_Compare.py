'''
2D Fusion:
1. Five-fold cross-validation + External testing
2. Early stopping based on Loss, select model with highest AUC on sub-validation set
3. Set learning rates specifically for each part
'''
import os
...
def train_with_hyperparams(params, device, train_loader, sub_val_loader, test_loader, external_test_loader, save_dir, args):
    ...
    if args.save_sign == "AUC":
        best_val_metric = 0.0  # Best AUC is larger
        metric_name = "AUC"
        is_better = lambda current, best: current > best + 1e-6  # AUC is larger
        patience = 5  # Early stopping patience
        patience_counter = 0  # Early stopping counter
    elif args.save_sign == "Loss":
        best_val_metric = float('inf')  # Best Loss is smaller
        metric_name = "Loss"
        is_better = lambda current, best: current < best - 1e-6  # Loss is smaller
        patience = 5  # Early stopping patience
        patience_counter = 0  # Early stopping counter
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Learning Rate: Backbone={current_lrs[0]:.2e}, Classifier={current_lrs[0]:.2e}, Other={current_lrs[1]:.2e}")
        ...
        # Draw sub-validation set confusion matrix
    ...
    # Draw test set confusion matrix
...
def main():
    ...
    if args.modality == "CEUS_2D_dinov3_HTCM_random":
        image_dirs['ceus'] = "./datasets/CEUS_select_frames/1_Crop_same_shape_avi_random_16frames"
    elif args.modality == "CEUS_2D_dinov3_HTCM_uniform":
    ...
    print(f"\nStart hyperparameter search, total {len(all_param_combinations)} parameter combinations")
